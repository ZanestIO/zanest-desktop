...
module.exports = {
    ...
    emit: ['cancel-edit-topic', 'refresh'],
    components: {},
    created() {
        ipcRenderer.send('getTopicInfo', {id: this.currentlyEditing.value})

        ipcRenderer.on('responseGetTopicInfo', (e, args) => {
            this.name.value = args.name
            this.level.value = args.level
            this.length.value = args.length
            this.id = args.topID

            if (args.desc) {
                this.desc.value = args.desc
            }
        })
    },
    methods: {
        ...
        addTopicInfo() {
            ...
            if (this.valid) {
                ...
                ipcRenderer.send('topicUpdate', args)
                ipcRenderer.on('responseTopicUpdate', (e, args) => {
                    if (args) {
          ...
          <div class="section-content">
            ...
            <div class="p-2 pt-2 flex">
              <a href="#"
                 ...
                 v-on:click="addTopicInfo">ویرایش</a>
              <a href="#"
                 ...
                 v-on:click="$emit('cancel-edit-topic')">انصراف</a>
